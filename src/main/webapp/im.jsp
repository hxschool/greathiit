...
<script>
        ...
        handleMessage:function (data) {
            ...
            switch (msg.type){
                case 'TYPE_TEXT_MESSAGE':
                    ...
                    break;
                case 'ONLINE_ON':
    	    	      layer.msg(message.username + "上线了！");
          	    	  layim.setFriendStatus(message.id, 'online');  
                    break;
                case 'ONLINE_OFF':
                	layer.msg(message.username + "下线了！");
    	    	      layim.setFriendStatus(message.id, 'offline');
                    break;
                default:
                    break;
            }
  ...
  layui.use('layim', function(layim){
    ...
    layim.config({
      ...
      //查看群员接口
      ,members: {
        url: '/im/api/group/getByGroupId'
        ,data: {id: im.getUid()}
      }

      ,uploadImage: {
        url: '/im/upload?t=img' //（返回的数据格式见下文）
      }

      ,uploadFile: {
        url: '/im/upload?t=file' //（返回的数据格式见下文）
      }
      ...
      ,msgbox: 'js/layui/css/modules/layim/html/msgbox.html' //消息盒子页面地址，若不开启，剔除该项即可
      ,chatLog: 'js/layui/css/modules/layim/html/chatlog.html' //聊天记录地址
      ,find: 'js/layui/css/modules/layim/html/find.html'
      //,copyright: true //是否授权
    });
